# Traffic Cat 桌面后台联网观察器技术实现文档（定稿）
...
当前技术落地以 `Linux` 为首发平台，`Windows` 与 `macOS` 后续沿用同一套产品结构与交互层，再分别补平台采集实现。

```text
┌──────────────────────────────────────────────────────────┐
│                      desktop-ui                          │
│  Tauri 主窗口 / 守望挂件 / 前端状态管理 / 交互桥接      │
└───────────────┬──────────────────────────────────────────┘
